...
const elements = {
  ...
  bonusHeading: document.querySelector("#bonus-heading"),
  ...
  gameState.options.forEach((pelican, index) => {
    ...
    button.innerHTML = `
      ...
      <span class="choice__caption">
        <span class="choice__model" aria-hidden="true">${pelican.model}</span>
...
function chooseAnswer(selectedButton, selectedPelican) {
  ...
  [...elements.choices.children].forEach((button) => {
    ...
    button.querySelector(".choice__model").removeAttribute("aria-hidden");
...
function showBonus() {
  ...
  window.scrollTo({ top: 0, behavior: "smooth" });
  elements.bonusHeading.focus({ preventScroll: true });
}
